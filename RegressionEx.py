...
X_train, y_train = create_toy_data(func=linear_func, interval=interval, sample_num=train_num, noise=noise, add_outlier=False)
X_test, y_test = create_toy_data(func=linear_func, interval=interval, sample_num=test_num, noise=noise, add_outlier=False)

X_train_large, y_train_large = create_toy_data(func=linear_func, interval=interval, sample_num=5000, noise=noise, add_outlier=False)
...
# 绘制数据
plt.scatter(X_train.numpy(), y_train.numpy(), marker='*', facecolor="none", edgecolor='#e4007f', s=50, label="train data")
